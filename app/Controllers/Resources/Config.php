...
class Config extends ResourceController
{
	public function lse_action()
    {
    	...
    	if(!empty($this->request->getPost('idLs'))){
	    	...
	    	if(in_array(session()->get('sess_role'), $arrRoleOperator)){
				if(in_array($lsModel->idJenisLS, $arrPermohonanOnline) && $lsModel->flagOnline == 'Y')
					$config->online_submission = true;
			}
			$btnAction = $this->form_action($statusProses,$lsModel);
    	}
    	else{
    		$btnAction = $this->form_action('','');
    	}
    	$btn['btn'] = $btnAction;
    	return $this->response->setJSON($btn);
    }

	private function form_action($statusProses,$lsModel)
	{
		$jenisLS			 = model('jenisLs')->select('id')->where('isIzinPersetujuan','Y')->get()->getResult();
    	$arrReqAlokasi = [];
    	foreach ($jenisLS as $key => $jenis) {
    		$arrReqAlokasi[$key] = $jenis->id;
    	}
		...
		// simpan dan kembalikan ke status reject (pelaku usaha)
		$btnAction->tolak = '<li aria-hidden="false" aria-disabled="false">
								<a class="btn btn-warning btn-sm my-1 btn-save" data-checkizin="N" data-action="'.encrypt_id('REJECT').'" href="javascript:void(0)" role="menuitem"> 
									<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-ban" width="15" height="15" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
										<path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 12m-9 0a9 9 0 1 0 18 0a9 9 0 1 0 -18 0" />
										<path d="M5.7 5.7l12.6 12.6" />
									</svg> Tolak 
		...
		// Request alokasi untuk LS yang memerlukan SPE
		$btnAction->reqalokasi = '';

		if(!empty($lsModel) && in_array($lsModel->idJenisLS, $arrReqAlokasi) && empty($lsModel->idReqAlokasi) ){
			$btnAction->reqalokasi = '<li aria-hidden="false" aria-disabled="false"><a class="badge bg-orange my-1 btn-reqalokasi" data-checkizin="Y" data-action="'.encrypt_id('SAVE').'" href="javascript:void(0)" role="menuitem"><i class="fa fa-star me-2"></i>Request Alokasi</a></li>';
			$btnAction->terbit = '';
		}
		else if(!empty($lsModel) && in_array($lsModel->idJenisLS, $arrReqAlokasi) && !empty($lsModel->idReqAlokasi) ){
			$btnAction->reqalokasi = '<li aria-hidden="false" aria-disabled="false"><a class="badge bg-orange my-1 btn-revisi-reqalokasi" data-checkizin="Y" data-action="'.encrypt_id('SAVE').'" href="javascript:void(0)" role="menuitem"><i class="fa fa-star me-2"></i>Revisi Request Alokasi</a></li>
										<li aria-hidden="false" aria-disabled="false"><a class="badge bg-purple my-1 btn-cancel-reqalokasi" data-checkizin="Y" data-action="'.encrypt_id('SAVE').'" href="javascript:void(0)" role="menuitem"><i class="fa fa-star me-2"></i>Batalkan Request Alokasi</a></li>';
		}
    	...
    	if(in_array($userRole,[3,6])) // operator
    	{
    		...
    		else if($statusProses == 'REJECT'){
    			$btnAction->tolak = $btnAction->reqalokasi = $btnAction->reject = $btnAction->simpan = $btnAction->terbit = '';
    		}

    		if(!empty($lsModel) && !empty($lsModel->idReqAlokasi) ){
	    		$btnAction->tolak = '';
	    	}
    	...
    	else if(in_array($userRole,[10])) // pelaku usaha
    	{
    		// if(!$config->online_submission)
    		// 	$btnAction->tolak = '';
    		...
    		// 	$btnAction->next = $btnAction->simpan = '';
    		// }
    		$btnAction->terbit = $btnAction->tolak = $btnAction->reqalokasi = '';
    	} else if($userRole == '' || empty($userRole)){
    	...
    	$btn = $btnAction->back.$btnAction->tolak.$btnAction->simpan.$btnAction->next.$btnAction->reqalokasi.$btnAction->terbit.$btnAction->cetak;
    	return $btn;
	}
}
