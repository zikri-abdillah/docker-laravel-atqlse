...
class Ls extends BaseController
{
    ...
    public function index($dataFilter)
    {
        ...
        if ($dataFilter == 'konsep') {
            $table_title        = 'Data LSE Batubara - Konsep ';
            $breadcrumb_active  = 'Konsep';
        } else if ($dataFilter == 'proses') {
            $table_title        = 'Data LSE Batubara - Proses';
            $breadcrumb_active  = 'Proses';
        } else if ($dataFilter == 'terbit') {
            $table_title        = 'Data LSE Batubara - Terbit';
        ...
        $param['content'] = $this->render('ekspor.batubara.ls.index', $page);
        ...
        $param['addJS'] .= '<script src="' . base_url() . '/js/ekspor/coal/coal.js?v=' . date('YmdHis') . '"></script>';
    ...
    public function input()
    {
        ...
        $param['addJS'] .= '<script src="' . base_url() . '/js/ekspor/coal/input.js?v=' . date('YmdHis') . '"></script>';
        $param['addJS'] .= '<script>var mandatoryFields = ' . json_encode($fieldsMandatory) . ' </script>';

        $param['content'] = $this->render('ekspor.batubara.ls.input', $page);
        return $this->render('layout.template', $param);
    }
    ...
    public function view()
    {
        ...
        $param['content'] = $this->render('ekspor.batubara.ls.view', $page);
        return $this->render('layout.template', $param);
    }
    ...
    public function edit()
    {
        try {
            ...
            $param['addJS'] .= '<script>var mandatoryFields = ' . json_encode($fieldsMandatory) . ' </script>';
            $param['addJS'] .= '<script src="' . base_url() . '/js/ekspor/coal/input.js?v=' . date('YmdHis') . '"></script>';
            $param['addJS'] .= '<script src="' . base_url() . '/js/ekspor/coal/edit.js?v=' . date('YmdHis') . '"></script>';
    ...
    public function list($dataFilter)
    {
        ...
        $lsModel        = model('tx_lseHdr');
        $arrData        = $lsModel->where('idJenisLS', 1)->where('statusProses <>', 'DELETED')->whereIn('statusProses', $castDataFilter[$dataFilter]);
        ...
        foreach ($arrData as $key => $data) {
            ...
            if (isset($pengajuan->nomorAju)) {
                $noAju = '<br><span style="color:blue">AJU : ' . $pengajuan->nomorAju . '</span>';
            ...
            $columns = [];
            $columns[] = $no++;
            $columns[] = '<span>' . $badgeJenisTerbit . '<br><span style="color:#1fa617;">' . $data->draftNo . '</span></span>' . $noAju . '<br><span style="color:#ff9324;">' . $jenisKomoditi . '</span><hr><span style="color:#00bcd4;">SI No: ' . $data->noSi . '</span><br><span>Cabang : ' . $data->namaCabang . '</span><br><span>Created : ' . reverseDateTime($data->created) . '</span>';
    ...
    private function cek_mandatory($data, $section, $act = '')
    {
        $errMandatory = [];
        if ($act == 'ISSUED')
            $mandatories = model('mandatory')->where('idJenisLS', 1)->where('section', $section)->findAll();
        else
            $mandatories = model('mandatory')->where('checkDraft', 'Y')->where('idJenisLS', 1)->where('section', $section)->findAll();
    ...
    public function act_button() {}

    public function edit_dok_nsw()
    {
        try {
            $id = decrypt_id($this->request->getPost('id'));
            $dokNsw = model('t_inswPermohonanDok');

            $dokNsw->select('tblPermohonanDok_pinsw.*,DATE_FORMAT(tblPermohonanDok_pinsw.tanggalDokumen, "%d-%m-%Y") as tglDok,DATE_FORMAT(tblPermohonanDok_pinsw.tglAkhirDokumen, "%d-%m-%Y") as tglAkhirDok,m_jenis_dokumen.id as idJenisDok,m_jenis_dokumen.jenisDokumen');
            $dokNsw->join('m_jenis_dokumen', 'm_jenis_dokumen.kodeInsw = tblPermohonanDok_pinsw.kodeDokumen', 'LEFT');
            $docs = $dokNsw->where('tblPermohonanDok_pinsw.id', $id)->first();

            $respData['docs'] = $docs;
            $resp = resp_success('', $respData);
            return $this->response->setJSON($resp);
        } catch (\Throwable $e) {
            $resp = resp_error('An Exception has occured!' . $e->getMessage());
            return $this->response->setJSON($resp);
    ...
    public function save_dok_nsw()
    {
        try {
            ...
            $tnggalAkhir    = reverseDate($data['tglAkhirDokumenNsw']);

            if (!empty($data['negaraPenerbitNsw'])) {
                $update['negaraPenerbit']       = $data['negaraPenerbitNsw'];
                $update['uraiNegaraPenerbit']   = model('negara')->where('kode', $data['negaraPenerbitNsw'])->first()->nama;
            }

            $update['tglAkhirDokumen']          =  $tnggalAkhir;

            // pengecekan tanggal akhir dokumen
            if ($tnggalAkhir < $tnggalSekarang) {
                $resp = resp_error('Tanggal akhir dokumen harus lebih dari tanggal sekarang.');
                return $this->response->setJSON($resp);
            }

            if ($dokNsw->where('id', $data['idDokNSW'])->set($update)->update())
                $resp = resp_success('', NULL);
            else
                $resp = resp_error('Dokumen gagal di update');
    ...
    public function create_perubahan()
    {
        try {
            ...
            $kaloris    = model('tx_lseKalori')->where('idLS', $idLs)->findAll();
            ...
            if ($dataLS) {
                ...
                $insert->draftNo        = DATE('y') . '/' . DATE('m') . '/' . PREFIX_NUMBERING . '/' . $noDraft;
                ...
                foreach ($kaloris as $key => $kalori) {
                    unset($kalori->id);
                    $kalori->idLs = $idHdr;
                    $this->db->table('tx_lse_kalori')->insert($kalori);
                }
