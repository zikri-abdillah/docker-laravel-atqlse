...
function flashalert(idtarget) {
    $('#' + idtarget).flash_message({
        text: 'Hanya angka dan koma (desimal)',
        how: 'append'
    });
}
...
function initnumberaja()
{
    ...
    $(".numberaja").keydown(function(e) {
        ...
        $disableKeys = 190; // enable if use formatangka_us()

        $disableKeys = 188; // enable if use formatangka()
        if ($.inArray(e.keyCode, [8, 9,35,36,37,39,46,116, $disableKeys]) !== -1 || (e.ctrlKey === true || e.metaKey === true)) {
            return;
        }
